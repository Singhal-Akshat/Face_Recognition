...
def info():
    with open('friendstrain.csv','r') as file: # opening the trained.csv file to get back the results
...
def recognize(test_image,size):
    ...
    img = cv2.cvtColor(imgS,cv2.COLOR_BGR2RGB)  # converting image from BGR to RGB
...
test_image = cv2.imread('Test\\friends1.jpg') # loading the image where we want to apply the face recognition
while True:
    source,test_image = cap.read()
    ...
    cv2.waitKey(10) # waiting till user presses a key 
