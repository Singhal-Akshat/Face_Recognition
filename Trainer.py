...
for image in myList:
    ...
    classnames.append(image.split(".")[0]) # getting image's name or we can say person's name
...
for image in images:
    ...
    encode_List.append(encode) # appending each encoding to our list

with open('friendstrain.csv','a',newline="") as file: # opening a .csv file for storing the encoding so that we don't have to find the encoding again and again
    writer= csv.writer(file) # making a writer object for writing in csv file
    for name,encode in zip(classnames,encode_List): # looping in classnames and encode list and storing that in our trained.csv file for future use
